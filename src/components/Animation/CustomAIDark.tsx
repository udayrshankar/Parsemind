...
const AgentModule = ({
  ...
  >
    ...
    <div className="flex flex-col">
      <span
        className={`text-xs font-bold tracking-wide uppercase transition-colors duration-500 ${
          isActive ? "text-indigo-200" : "text-neutral-300"
        }`}
      >
        {label}
      </span>
      ...
      <div className="absolute top-6 left-0 right-0 z-30 flex flex-col items-center justify-center pointer-events-none">
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background:
                "conic-gradient(from 90deg at 50% 50%, #E5E5E5 0%, #6366f1 50%, #E5E5E5 100%)",
            }}
          />

          {/* LAYER 2: The Content */}
          {/* 👇 FIX APPLIED HERE: Added 'pointer-events-auto' and 'cursor-pointer' */}
          <div
            className="relative flex h-full w-full items-center justify-center gap-2 bg-white/95 px-4 py-2 backdrop-blur-md pointer-events-auto cursor-pointer"
